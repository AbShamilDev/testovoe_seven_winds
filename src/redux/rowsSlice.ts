...
export const fetchRows = createAsyncThunk("rowsSlice/fetchRows", async () => {
  const response = await axios.get("/outlay-rows/entity/128732/row/list");
...
export const deleteRowRequest = createAsyncThunk(
  ...
  async (rowId: number) => {
    const response = await axios.delete(`/outlay-rows/entity/128732/row/${rowId}/delete`);
    return response.data;
  }
);
...
export const updateRowRequest = createAsyncThunk(
  ...
  async ({ rowId, row }: { rowId: number; row: Omit<rowType, "id" | "child" | "total"> }) => {
    const response = await axios.post(`/outlay-rows/entity/128732/row/${rowId}/update`, row, {
      headers: { "Content-Type": "application/json" },
    });
    return response.data;
  }
